...
class TestFPLAPI(unittest.TestCase):
    ...
    def test_my_team_structure(self):
        """Test /my-team endpoint data structure"""
        try:
            ...
            # Validate team value and ensure it's reasonable
            self.assertGreater(data['last_deadline_value'], 0)
            team_value = data['last_deadline_value'] / 10
            self.assertTrue(80 <= team_value <= 120, 
                f"Team value £{team_value}m seems unreasonable")
            
            print("\nTeam Data Validation:")
            print(f"Team Value: £{team_value}m")
            print(f"Free Transfers: {data['transfers']['limit']}")
            print(f"Bank Value: £{data['last_deadline_bank']/10}m")
